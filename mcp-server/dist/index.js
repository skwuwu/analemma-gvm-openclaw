...
server.registerTool("gvm_declare_intent", {
    title: "GVM Declare Intent",
    description: "Declare what you intend to do BEFORE making an API call. " +
...
server.registerTool("gvm_load_rulesets", {
    title: "GVM Load Rulesets",
    description: "Auto-detect installed OpenClaw skills and load matching GVM governance rulesets. " +
    ...
    let available = [];
    try {
        available = readdirSync(rulesetsDir).filter((f) => f.endsWith(".toml") && !f.startsWith("_"));
    }
    catch {
        // ignore
    }
    return ok(JSON.stringify({
        ...
        no_ruleset: no_ruleset.length > 0
            ? {
                skills: no_ruleset,
                action: "These skills have no matching ruleset. Their domains will use " +
