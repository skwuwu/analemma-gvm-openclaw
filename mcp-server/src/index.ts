...
server.registerTool(
  "gvm_status",
  {
    title: "GVM Status",
    description:
      "Show current GVM security status: shadow mode, active rulesets, " +
  ...
  async () => {
    ...
    const shadow = info.shadow as Record<string, unknown> | undefined;
    const registry = info.registry as Record<string, unknown> | undefined;
...
server.registerTool(
  "gvm_audit_log",
  {
    title: "GVM Audit Log",
    description:
      "View recent governance decisions from the WAL audit log. " +
  ...
  async (args) => {
      ...
      try {
        ...
        for (const line of lines) {
          try {
            ...
            if (evt.event_id) events.push(evt);
          } catch {
    ...
    if (filter !== "all") {
      events = events.filter((e) => {
        ...
        if (filter === "denied") return d === "deny" || d.includes("deny");
        if (filter === "delayed") return d.includes("delay");
        if (filter === "allowed") return d === "allow";
        return true;
      });
    }
    ...
    const formatted = events.map((e) => ({
      ...
      target: `${e.transport && (e.transport as Record<string, unknown>).method || "?"} ${e.transport && (e.transport as Record<string, unknown>).host || "?"}${e.transport && (e.transport as Record<string, unknown>).path || ""}`,
      agent: e.agent_id,
      status: e.status,
    }));
...
server.registerTool(
  "gvm_blocked_summary",
  {
    ...
    description:
      "Human-readable summary of governance activity. " +
  ...
  async (args) => {
    ...
    for (const e of events) {
      ...
      if (d === "allow") {
        ...
        denied++;
        const transport = e.transport as Record<string, unknown> | undefined;
        deniedDetails.push(
          `${transport?.method ?? "?"} ${transport?.host ?? "?"}${transport?.path ?? ""} — ${e.operation ?? "unknown"}`,
        );
      }
    }
...
server.registerTool(
  ...
  async (args) => {
    ...
    if (existsSync(defaultPath)) {
      sections.push(
        "# GVM SRR Rules (generated by gvm_select_rulesets)\n" +
